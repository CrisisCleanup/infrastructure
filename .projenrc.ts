...
import { ExtensionMatch, GitHooks, LintStaged, ValidGitHooks } from "@arroyodev-llc/projen.component.git-hooks";
...
// Setup githooks
monorepo.applyRecursive((project) => {
  if (project instanceof typescript.TypeScriptProject) {
    new LintStaged(project, {
      entries: [
        {
          extensions: [ExtensionMatch.TS, ExtensionMatch.JS],
          commands: [NodePackageUtils.command.exec(project.package.packageManager, 'eslint --cache --fix --no-error-on-unmatched-pattern')]
        },
        {
          extensions: [ExtensionMatch.YAML],
          commands: [NodePackageUtils.command.exec(project.package.packageManager, 'prettier --write')]
        }
      ]
    })
  }
  // ensure all readonly files are ignored
  project.files.filter((file) => file.readonly).forEach((file) => {
...
new GitHooks(monorepo, {
  hooks: {
    [ValidGitHooks.PreCommit]: NodePackageUtils.command.exec(monorepo.package.packageManager, 'lint-staged')
  },
  preserveUnused: true
})
